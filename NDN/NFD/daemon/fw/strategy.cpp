                        ...
                        const Face& inFace, const Data& data)
{
  ...
  int wirelessCount = -1;
  float rateThreshold = 999;
  
  for (const auto& pendingDownstream : pendingDownstreams) {
    if (pendingDownstream->getLocalUri().getHost() == "wlx0013eff9002c") {
      wirelessCount += 1;
      float rate = get_rate(pendingDownstream->getLocalUri().getHost(), pendingDownstream->getRemoteUri().getHost());
      if (rate < rateThreshold)
        rateThreshold = rate;
      if (pendingDownstream->getRemoteUri().getHost() == "01:00:5e:00:17:aa" || wirelessCount > 0) {
        wirelessCount += 1;
        break;
      }
    }
  ...
  if (wirelessCount > 0) {
    set_rate(this->getWirelessMulticastEtherFace()->getLocalUri().getHost(), rateThreshold);
    this->sendData(pitEntry, data, *(this->getWirelessMulticastEtherFace()));
    for (const auto& pendingDownstream : pendingDownstreams)
      if (pendingDownstream->getLocalUri().getHost() != "wlx0013eff9002c")
