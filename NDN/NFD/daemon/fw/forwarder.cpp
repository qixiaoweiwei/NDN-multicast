...
Forwarder::onIncomingData(const FaceEndpoint& ingress, const Data& data)
{
  ...
  if (pitMatches.size() == 1) {
    ...
    if (Name("File").isPrefixOf(data.getName())) {
      int segmentCount = std::stoi(reinterpret_cast<const char*>(data.getContent().value()));
      std::string fileName = data.getName().getSubName(1, 1).toUri();
      for (int segment = 0; segment < segmentCount; segment++) {
        Name interestName = Name("/Segment"+fileName+"/"+std::to_string(segment));
        auto interest = make_shared<Interest>(interestName);
        interest->setCanBePrefix(false);
        interest->setInterestLifetime(200_s);
        shared_ptr<pit::Entry> localPitEntry = m_pit.insert(*interest).first;
        for (const pit::InRecord& inRecord : pitEntry->getInRecords())
          localPitEntry->insertOrUpdateInRecord(inRecord.getFace(), *interest);
      }
    }
