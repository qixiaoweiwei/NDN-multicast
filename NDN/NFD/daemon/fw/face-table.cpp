...
FaceTable::addImpl(shared_ptr<Face> face, FaceId faceId)
{
  ...
  if (face->getLocalUri().getHost() == "wlx0013eff9002c" && face->getRemoteUri().getHost() == "01:00:5e:00:17:aa") {
    m_wireless_ether_multicast_face = face.get();
    NFD_LOG_INFO("Recorded wireless ethernet multicast face id=" << faceId <<
               " remote=" << m_wireless_ether_multicast_face->getRemoteUri());  
  }
