	...
	if (!tcb_desc->disable_ratefallback || !tcb_desc->use_driver_rate) {
		if (mac->opmode == NL80211_IFTYPE_STATION) {
			...
			if (tcb_desc->multicast || tcb_desc->broadcast) {
				...
				else if (kiwi_rate_idx == 2)
					tcb_desc->hw_rate = rtlpriv->cfg->maps[RTL_RC_CCK_RATE5_5M];
				else if (kiwi_rate_idx == 3)
					tcb_desc->hw_rate = rtlpriv->cfg->maps[RTL_RC_CCK_RATE11M];
				else if (kiwi_rate_idx == 4)
					tcb_desc->hw_rate = rtlpriv->cfg->maps[RTL_RC_OFDM_RATE6M];
				else if (kiwi_rate_idx == 5)
					tcb_desc->hw_rate = rtlpriv->cfg->maps[RTL_RC_OFDM_RATE9M];
				else if (kiwi_rate_idx == 6)
					tcb_desc->hw_rate = rtlpriv->cfg->maps[RTL_RC_OFDM_RATE12M];
				else if (kiwi_rate_idx == 7)
					tcb_desc->hw_rate = rtlpriv->cfg->maps[RTL_RC_OFDM_RATE18M];
				else if (kiwi_rate_idx == 8)
					tcb_desc->hw_rate = rtlpriv->cfg->maps[RTL_RC_OFDM_RATE24M];
				else if (kiwi_rate_idx == 9)
					tcb_desc->hw_rate = rtlpriv->cfg->maps[RTL_RC_OFDM_RATE36M];
				else if (kiwi_rate_idx == 10)
					tcb_desc->hw_rate = rtlpriv->cfg->maps[RTL_RC_OFDM_RATE48M];
				else if (kiwi_rate_idx == 11)
...
#define SET_RATE_ID(rate_id)					\
	...
	if (ieee80211_is_data(fc)) {
		...
		_rtl_txrate_selectmode(hw, sta, tcb_desc, info->control.rates[0].idx);
