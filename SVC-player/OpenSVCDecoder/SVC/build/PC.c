...
int readh264_init_kiwi (char *h264_file);
...
void WriteYUVCrop(FILE *ptfile, int PicWidth, int PicHeight, unsigned char *Y, unsigned char *U, unsigned char *V);
...
int readh264_init_kiwi (char *h264_file);
void get_layer_kiwi(int layer, ConfigSVC *Config);

int decode(char *h264_file, char *out_file, int highest_layer, int frames) {
  FILE *ptfile = fopen(out_file, "wb");
  int count = 0;
  
  memset(Display_1_Extract_1_Image_Y_o, 0, sizeof(image_type)*3279368);
  memset(Display_1_Extract_Image_Y_o, 0, sizeof(image_type)*3279368);
  memset(DqIdNextNal_Nal_o, 0, sizeof(NAL)*1);
  memset(GetNalBytesAu_NalStreamer, 0, sizeof(NALSTREAMER)*1);
  memset(GetNalBytesAu_PPS, 0, sizeof(PPS)*255);
  memset(GetNalBytesAu_SPS, 0, sizeof(SPS)*32);
  memset(GetNalBytesAu_StreamType, 0, sizeof(ConfigSVC)*1);
  memset(GetNalBytesAu_rbsp_o_size, 0, sizeof(int)*1);
  memset(GetNalBytesAu_rbsp_o, 0, sizeof(stream)*1622016);
  memset(ReadAU_DataFile_o, 0, sizeof(uchar)*1622016);
  memset(ReadAU_ReadBytes, 0, sizeof(int)*1);
  memset(ReadAU_pos_o, 0, sizeof(int)*1);
  memset(decoder_svc_MvBuffer_1_Mv, 0, sizeof(short)*11489280);
  memset(decoder_svc_MvBuffer_1_Ref, 0, sizeof(short)*5744640);
  memset(decoder_svc_MvBuffer_Mv, 0, sizeof(short)*11489280);
  memset(decoder_svc_MvBuffer_Ref, 0, sizeof(short)*5744640);
  memset(decoder_svc_NalUnit_NalRefIdc_io, 0, sizeof(int)*1);
  memset(decoder_svc_NalUnit_NalUnitType_io, 0, sizeof(int)*1);
  memset(decoder_svc_Nal_Compute_NalDecodingProcess_Set_Pos_Pos, 0, sizeof(int)*1);
  memset(decoder_svc_Nal_Compute_NalDecodingProcess_SliceHeader_entropy_coding_flag, 0, sizeof(int)*1);
  memset(decoder_svc_Nal_Compute_NalDecodingProcess_SliceHeader_pps_id, 0, sizeof(PPS)*1);
  memset(decoder_svc_Nal_Compute_NalDecodingProcess_SliceHeader_quantif, 0, sizeof(W_TABLES)*1);
  memset(decoder_svc_Nal_Compute_NalDecodingProcess_SliceHeader_sps_id, 0, sizeof(SPS)*1);
  memset(decoder_svc_Nal_Compute_NalDecodingProcess_Slice_type_SliceType_o, 0, sizeof(int)*1);
  memset(decoder_svc_Nal_Compute_NalDecodingProcess_slice_layer_CABAC_mv_cabac_l0_o, 0, sizeof(short)*261120);
  memset(decoder_svc_Nal_Compute_NalDecodingProcess_slice_layer_CABAC_mv_cabac_l1_o, 0, sizeof(short)*261120);
  memset(decoder_svc_Nal_Compute_NalDecodingProcess_slice_layer_CABAC_ref_cabac_l0_o, 0, sizeof(short)*32640);
  memset(decoder_svc_Nal_Compute_NalDecodingProcess_slice_layer_CABAC_ref_cabac_l1_o, 0, sizeof(short)*32640);
  memset(decoder_svc_Nal_Compute_SetPos_Pos, 0, sizeof(int)*1);
  memset(decoder_svc_Nal_Compute_SliceHeaderIDR_entropy_coding_flag, 0, sizeof(int)*1);
  memset(decoder_svc_Nal_Compute_SliceHeaderIDR_pps_id, 0, sizeof(PPS)*1);
  memset(decoder_svc_Nal_Compute_SliceHeaderIDR_quantif, 0, sizeof(W_TABLES)*1);
  memset(decoder_svc_Nal_Compute_SliceHeaderIDR_sps_id, 0, sizeof(SPS)*1);
  memset(decoder_svc_Nal_Compute_nal_unit_header_svc_ext_20_pos_o, 0, sizeof(int)*1);
  memset(decoder_svc_Nal_Compute_sei_rbsp_Sei, 0, sizeof(SEI)*1);
  memset(decoder_svc_Nal_Compute_seq_parameter_set_IdOfsps_o, 0, sizeof(int)*1);
  memset(decoder_svc_Nal_Compute_slice_layer_in_scalable_ext_20_Decode_IPB_svc_Decode_B_svc_Upsampling_tmp, 0, sizeof(short)*2088960);
  memset(decoder_svc_Nal_Compute_slice_layer_in_scalable_ext_20_Decode_IPB_svc_Decode_B_svc_px, 0, sizeof(short)*1920);
  memset(decoder_svc_Nal_Compute_slice_layer_in_scalable_ext_20_Decode_IPB_svc_Decode_B_svc_py, 0, sizeof(short)*1088);
  memset(decoder_svc_Nal_Compute_slice_layer_in_scalable_ext_20_Decode_IPB_svc_Decode_B_svc_xk16, 0, sizeof(short)*1920);
  memset(decoder_svc_Nal_Compute_slice_layer_in_scalable_ext_20_Decode_IPB_svc_Decode_B_svc_xp16, 0, sizeof(short)*1920);
  memset(decoder_svc_Nal_Compute_slice_layer_in_scalable_ext_20_Decode_IPB_svc_Decode_B_svc_yk16, 0, sizeof(short)*1088);
  memset(decoder_svc_Nal_Compute_slice_layer_in_scalable_ext_20_Decode_IPB_svc_Decode_B_svc_yp16, 0, sizeof(short)*1088);
  memset(decoder_svc_Nal_Compute_slice_layer_in_scalable_ext_20_Decode_IPB_svc_Decode_P_svc_Upsampling_tmp, 0, sizeof(short)*2088960);
  memset(decoder_svc_Nal_Compute_slice_layer_in_scalable_ext_20_Decode_IPB_svc_Decode_P_svc_px, 0, sizeof(short)*1920);
  memset(decoder_svc_Nal_Compute_slice_layer_in_scalable_ext_20_Decode_IPB_svc_Decode_P_svc_py, 0, sizeof(short)*1088);
  memset(decoder_svc_Nal_Compute_slice_layer_in_scalable_ext_20_Decode_IPB_svc_Decode_P_svc_xk16, 0, sizeof(short)*1920);
  memset(decoder_svc_Nal_Compute_slice_layer_in_scalable_ext_20_Decode_IPB_svc_Decode_P_svc_xp16, 0, sizeof(short)*1920);
  memset(decoder_svc_Nal_Compute_slice_layer_in_scalable_ext_20_Decode_IPB_svc_Decode_P_svc_yk16, 0, sizeof(short)*1088);
  memset(decoder_svc_Nal_Compute_slice_layer_in_scalable_ext_20_Decode_IPB_svc_Decode_P_svc_yp16, 0, sizeof(short)*1088);
  memset(decoder_svc_Nal_Compute_slice_layer_in_scalable_ext_20_Slice_Layer_SliceLayerCabac_mv_cabac_l0_o, 0, sizeof(short)*261120);
  memset(decoder_svc_Nal_Compute_slice_layer_in_scalable_ext_20_Slice_Layer_SliceLayerCabac_mv_cabac_l1_o, 0, sizeof(short)*261120);
  memset(decoder_svc_Nal_Compute_slice_layer_in_scalable_ext_20_Slice_Layer_SliceLayerCabac_ref_cabac_l0_o, 0, sizeof(short)*32640);
  memset(decoder_svc_Nal_Compute_slice_layer_in_scalable_ext_20_Slice_Layer_SliceLayerCabac_ref_cabac_l1_o, 0, sizeof(short)*32640);
  memset(decoder_svc_Nal_Compute_slice_layer_in_scalable_ext_20_Slice_type_SliceType_o, 0, sizeof(int)*1);
  memset(decoder_svc_Nal_Compute_slice_layer_in_scalable_ext_20_slice_header_21_entropy_coding_flag, 0, sizeof(int)*1);
  memset(decoder_svc_Nal_Compute_slice_layer_in_scalable_ext_20_slice_header_21_pps_id, 0, sizeof(PPS)*1);
  memset(decoder_svc_Nal_Compute_slice_layer_in_scalable_ext_20_slice_header_21_quantif, 0, sizeof(W_TABLES)*1);
  memset(decoder_svc_Nal_Compute_slice_layer_in_scalable_ext_20_slice_header_21_sps_id, 0, sizeof(SPS)*1);
  memset(decoder_svc_Nal_Compute_slice_layer_main_CABAC_mv_cabac_l0_o, 0, sizeof(short)*261120);
  memset(decoder_svc_Nal_Compute_slice_layer_main_CABAC_mv_cabac_l1_o, 0, sizeof(short)*261120);
  memset(decoder_svc_Nal_Compute_slice_layer_main_CABAC_ref_cabac_l0_o, 0, sizeof(short)*32640);
  memset(decoder_svc_Nal_Compute_slice_layer_main_CABAC_ref_cabac_l1_o, 0, sizeof(short)*32640);
  memset(decoder_svc_PictureBuffer_RefU, 0, sizeof(uchar)*12570880);
  memset(decoder_svc_PictureBuffer_RefV, 0, sizeof(uchar)*12570880);
  memset(decoder_svc_PictureBuffer_RefY, 0, sizeof(uchar)*50283520);
  memset(decoder_svc_ResiduBuffer_RefU, 0, sizeof(short)*2186240);
  memset(decoder_svc_ResiduBuffer_RefV, 0, sizeof(short)*2186240);
  memset(decoder_svc_ResiduBuffer_RefY, 0, sizeof(short)*8744960);
  memset(decoder_svc_Residu_Block, 0, sizeof(DATA)*8160);
  memset(decoder_svc_Residu_Current_pic, 0, sizeof(LIST_MMO)*1);
  memset(decoder_svc_Residu_MbToSliceGroupMap, 0, sizeof(uchar)*8160);
  memset(decoder_svc_Residu_Mmo, 0, sizeof(MMO)*1);
  memset(decoder_svc_Residu_PPS, 0, sizeof(PPS)*255);
  memset(decoder_svc_Residu_RefL0, 0, sizeof(LIST_MMO)*16);
  memset(decoder_svc_Residu_RefL1, 0, sizeof(LIST_MMO)*16);
  memset(decoder_svc_Residu_Residu, 0, sizeof(RESIDU)*32640);
  memset(decoder_svc_Residu_SPS, 0, sizeof(SPS)*32);
  memset(decoder_svc_Residu_Slice, 0, sizeof(SLICE)*1);
  memset(decoder_svc_Residu_SliceGroupId, 0, sizeof(uchar)*8160);
  memset(decoder_svc_Residu_SliceTab, 0, sizeof(uchar)*8160);
  memset(decoder_svc_Svc_Vectors_PC_Svc_Vectors, 0, sizeof(SVC_VECTORS)*1);
  memset(decoder_svc_VideoParameter_Crop, 0, sizeof(int)*1);
  memset(decoder_svc_VideoParameter_EndOfSlice, 0, sizeof(int)*1);
  memset(decoder_svc_VideoParameter_ImgToDisplay, 0, sizeof(int)*1);
  memset(decoder_svc_VideoParameter_address_pic_o, 0, sizeof(int)*1);
  memset(decoder_svc_VideoParameter_xsize_o, 0, sizeof(int)*1);
  memset(decoder_svc_VideoParameter_ysize_o, 0, sizeof(int)*1);
  memset(decoder_svc_VlcTab_PC_o, 0, sizeof(VLC_TABLES)*1);
  memset(decoder_svc_slice_main_vector_PC_Main_vector_o, 0, sizeof(MAIN_STRUCT_PF)*1);
  ...
  *ReadAU_ReadBytes = 1622016;
  readh264_init_kiwi(h264_file);
  get_layer_kiwi(highest_layer, GetNalBytesAu_StreamType);
      ...
      case 1 : {/* case_48 */
        {
           ...
           uchar *U = Y + (XDIM + 32) * YDIM;
           uchar *V = U + (XDIM + 32) * YDIM/4 ;
           WriteYUVCrop(ptfile, XDIM, YDIM, Y, U, V);
        }
